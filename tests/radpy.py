...
def projop(spins, state):
    ...
    match state:
        ...
        case "Tpm": 
            return (2 * SAz**2 + SAz) * (2 * SBz**2 + SBz) + (2 * SAz**2 - SAz) * (2 * SBz**2 - SBz)
        case "Eq": 
...
def HamiltonianHyperfine(spins, pos, pos2, HFC):
    omega = HFC * 1.76e8
...
def HamiltonianDipolar3D(DipolarInteractions):
    ...
    return sum([t[0] * np.kron(np.kron(t[1], t[2]), t[3]) 
                if ds == 3 else
                np.kron(t[1], t[2]) for t in terms])
...
def Kinetics(spins, k=0, time=0, model="Haberkorn-singlet"):
    ...
    match model:
        ...
        case "Haberkorn-singlet":
            return 0.5 * k * (np.kron(QS, np.eye(len(QS))) + (np.kron(np.eye(len(QS)), QS)))
        case "Haberkorn-triplet":
            return 0.5 * k * (np.kron(QT, np.eye(len(QT))) + (np.kron(np.eye(len(QT)), QT)))
        case "Haberkorn-free":
            return k * np.kron(np.eye(len(QS)), np.eye(len(QS)))
        case "Jones-Hore":
            return 0.5 * k * (np.kron(QS, np.eye(len(QS))) + (np.kron(np.eye(len(QS)), QS))) + 0.5 * kt * (np.kron(QT, np.eye(len(QT))) + (np.kron(np.eye(len(QT)), QT))) + (0.5 * (ks + kt)) * (np.kron(QS, QT) + np.kron(QT, QS))
...
def Relaxation(spins, k=0, model="ST-Dephasing"):
    ...
    match model:
        ...
        case "TT-Dephasing":
            return k * (np.kron(QTp, QTm) + np.kron(QTm, QTp) + np.kron(QT0, QTm) + np.kron(QTm, QT0) + np.kron(QTp, QT0) + np.kron(QT0, QTp))
        case "RandomFields":
            return k * (1.5 * np.kron(np.eye(len(QS)), np.eye(len(QS))) - np.kron(SAx, SAx.T) - np.kron(SAy, SAy.T) - np.kron(SAz, SAz.T) - np.kron(SBx, SBx.T) - np.kron(SBy, SBy.T) - np.kron(SBz, SBz.T))
        case "DipolarModulation":
            return k * (1 / 9 * np.kron(QS, QTp) + 1 / 9 * np.kron(QTp, QS) + 1 / 9 * np.kron(QS, QTm) + 1 / 9 * np.kron(QTm, QS) + 4 / 9 * np.kron(QS, QT0) + 4 / 9 * np.kron(QT0, QS) + np.kron(QTp, QT0) + np.kron(QT0, QTp) + np.kron(QTm, QT0) + np.kron(QT0, QTm))
...
def Hilbert_observable(state, spins):
    ...
    if np.array_equal(Pobs, projop(spins, "T")):
        obs = 1 - np.real(np.trace(np.matmul(projop(spins, "S"), (projop(spins, "S") / np.trace(projop(spins, "S"))))))
    else:
...
def Liouville_observable(state, spins):
    ...
    if np.array_equal(Pobs, projop_Liouville(spins, "T")):
        obs = 1 - np.real(np.trace(np.matmul(projop_Liouville(spins, "S").T, 
                                             (projop_Liouville(spins, "S") / np.vdot(projop_Liouville(spins, "S"), projop_Liouville(spins, "S"))))))
    else:
...
def TimeEvolution(spins, initial, observable, t_max, t_stepsize, k, B, H, space="Hilbert", model="Exponential"):
...
def MARY(spins, initial, observable, t_max, t_stepsize, k, B, Hplot, space="Hilbert"):
    ...
    for i,B0 in enumerate(B):
        time, MFE[i,:], productyield, ProductYieldSum, rhot = TimeEvolution(spins, initial, observable, t_max, t_stepsize, k, B0, Hplot, space=space)
...
def T1_T2_RelaxationTimes(g_tensors, B, tau_c):
    ...
    g_innerproduct = (g_tensors[0] - g_mean)**2 + (g_tensors[1] - g_mean)**2 + (g_tensors[2] - g_mean)**2
    omega = gamma * B
    
    T1 = ((1 / 5) * ((muB * B) / hbar)**2 * g_innerproduct * (tau_c / (1 + omega**2 * tau_c**2)))**(-1)
    T2 = ((1 / 30) * ((muB * B) / hbar)**2 * g_innerproduct * (4 * tau_c + (3 * tau_c / (1 + omega**2 * tau_c**2))))**(-1)
    return [T1, T2]
...
def EffectiveHyperfine(radical_hfc, radical_spin):
    ...
    return np.sqrt((4 / 3) * sum((radical_HFC**2 * spin_quantum_number) * (spin_quantum_number + 1)))


def BhalfTheoretical(radicalA_effectiveHFC, radicalB_effectiveHFC):
    return np.sqrt(3) * ((radicalA_effectiveHFC**2 + radicalB_effectiveHFC**2) / (radicalA_effectiveHFC + radicalB_effectiveHFC))
...
def SingletYieldTwoSite(B, gamma, theta, phi, r12A, r13A, r23A, r12B, r13B, r23B, 
                                       kS, kF, tau, spins):
    ...
    DipolarInteractions = [
        [DipolarInteraction(r, gamma) for r in r_distance]
    ...
    LA = Hilbert2Liouville(HA) + Kinetics(spins, kS, 0, model="Haberkorn-singlet") + K[0]
    LB = Hilbert2Liouville(HB) + Kinetics(spins, kS, 0, model="Haberkorn-singlet") + K[1]
...
def SingletYieldTwoSiteRelaxation(B, gamma, theta, phi, r12A, r13A, r23A, r12B, r13B, r23B, 
                                       kS, kF, tau, spins):
    ...
    H = HZ + DD
    K = Kinetics(spins, kS, 0, model="Haberkorn-singlet") + Kinetics(spins, kF, 0, model="Haberkorn-free")
    L = Hilbert2Liouville(H) + K
    
    relaxation13 = 0.25 * tau * gamma**2 * (DipolarInteraction1D(r13A) - DipolarInteraction1D(r13B))**2
    relaxation23 = 0.25 * tau * gamma**2 * (DipolarInteraction1D(r23A) - DipolarInteraction1D(r23B))**2
    
    h13 = projop_3spin(spins, 0, 2, "Tp") / 3 + projop_3spin(spins, 0, 2, "Tm") / 3 - 2 * projop_3spin(spins, 0, 2, "T0") / 3
    h23 = projop_3spin(spins, 1, 2, "Tp") / 3 + projop_3spin(spins, 1, 2, "Tm") / 3 - 2 * projop_3spin(spins, 1, 2, "T0") / 3

    relaxh13 = np.dot(h13, h13)
    relaxh23 = np.dot(h23, h23)
    
    R = (relaxation13 * (np.kron(relaxh13, np.eye(len(relaxh13))) + np.kron(np.eye(len(relaxh13)), relaxh13.T) - 
                        2 * np.kron(h13, h13.T))) + (relaxation23 * (np.kron(relaxh23, np.eye(len(relaxh23))) + 
                       np.kron(np.eye(len(relaxh23)), relaxh23.T) - 2 * np.kron(h23, h23.T)))
    L += R
    
    singletyield = (kS / M) * (np.dot(np.ndarray.flatten(P12S), np.linalg.solve(L, np.ndarray.flatten(rho0))))

    return singletyield


def SingletYieldTwoSiteApprox(B, gamma, theta, phi, r12A, r13A, r23A, r12B, r13B, r23B, 
                                       k, kAB, kBA, tmax, tstep, spins):
...
def SingletYieldTwoSiteTimePropagation(B, gamma, theta, phi, r12A, r13A, r23A, r12B, r13B, r23B, 
                                       k, kAB, kBA, tmax, tstep, spins):
...
def SingletYieldTwoSiteTimePropagationApprox(B, gamma, theta, phi, r12A, r13A, r23A, r12B, r13B, r23B, 
                                       k, kAB, kBA, tmax, tstep, spins):
...
def TimeEvolutionPlot2D(t_max, x, y, markercolour, y2, markercolour2, xlabel, ylabel, title):
    fig = plt.figure()
    figure = plt.gcf()
...
def DensityMatrixPlot2D(rhot, x_axis_labels, y_axis_labels, space="Hilbert", colourmap="viridis"):
    ...
    match space:
        case "Hilbert":
            ...
            sns.heatmap(np.abs(rhot), annot=True, linewidths=.5, cmap=colourmap, 
                        cbar=False, xticklabels=x_axis_labels, yticklabels=y_axis_labels)
        ...
        case "Liouville":
            ...
            x_axis_labels = x_axis_labels
            y_axis_labels = y_axis_labels
            sns.heatmap(np.abs(np.reshape(rhotL, (dims, dims))), annot=True, linewidths=.5, cmap=colourmap, 
                        cbar=False, xticklabels=x_axis_labels, yticklabels=y_axis_labels)
            figure.set_size_inches(5, 5)
            plt.show()
...
def randomwalk3D(n_steps, x_0, y_0, z_0, mut_D, del_T):
    ...
    x, y, z, dist, angle = np.zeros(n_steps), np.zeros(n_steps), np.zeros(n_steps), np.zeros(n_steps), np.zeros(n_steps + 1)
    ...
    for i in range(1, n_steps):
        ...
        dist_sq = (x[i] + x[i -1])**2 + (y[i] + y[i - 1])**2 + (z[i] + z[i - 1])**2 
...
def FAD_Bzero(C, t, kbet, pH, khfc, kd, k1, km1, krt):
    ...
    dRTpmdt = 2 * khfc * Crs - (km1 * Hp + 2 * khfc) * Crtpm + 2 * khfc * Crt0 + k1 * Ctpm
