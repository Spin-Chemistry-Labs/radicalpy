...
def main(Bmax=4, Bmin=0, dB=0.1, tmax=3e-6, dt=10e-9):
    flavin = rp.simulation.Molecule.fromdb("flavin_anion", ["N5"])  # , "H27", "H29"])
    trp = rp.simulation.Molecule.fromdb("tryptophan_cation", ["Hbeta1"])  # , "Hbeta1"])
    sim = rp.simulation.LiouvilleSimulation([flavin, trp])
    time = np.arange(0, tmax, dt)
    B0 = np.arange(Bmin, Bmax, dB)
    B1 = 0.1
    B1_freq = rp.utils.MHz_to_mT(15)
    krec = 1.1e7
    kesc = 7e6
    kSTD = 0
    kr = 0
    ...
    rydmr_result = results["product_yield_sums"] - results["product_yield_sums"][0]
